...
-- Alternative solution by one of the users
/*
SELECT CT.CONTEST_ID, CT.HACKER_ID, CT.NAME, SUM(TS), SUM(TAS), SUM(TV), SUM(TUV) FROM CONTESTS CT INNER JOIN COLLEGES AS CL ON CL.CONTEST_ID = CT.CONTEST_ID INNER JOIN CHALLENGES AS CH ON CH.COLLEGE_ID = CL.COLLEGE_ID LEFT JOIN ( SELECT CHALLENGE_ID, SUM(TOTAL_SUBMISSIONS) AS TS, SUM(TOTAL_ACCEPTED_SUBMISSIONS) AS TAS FROM SUBMISSION_STATS GROUP BY CHALLENGE_ID ) S ON S.CHALLENGE_ID = CH.CHALLENGE_ID LEFT JOIN ( SELECT CHALLENGE_ID, SUM(TOTAL_VIEWS) AS TV, SUM(TOTAL_UNIQUE_VIEWS) AS TUV FROM VIEW_STATS GROUP BY CHALLENGE_ID ) V ON V.CHALLENGE_ID = CH.CHALLENGE_ID GROUP BY CT.CONTEST_ID ORDER BY CT.CONTEST_ID;*/
